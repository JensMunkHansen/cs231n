...
def affine_backward(dout, cache):
  ...
  ###########################################################################
  # f(x,w) = w x + b
  # L(W) = 1/N L_i(f(x_i,W),y_i) + lambda R(W)
  #
  # SVM: L_i = sum_{j neq y_i} max(0, s_j - s_yi + 1)
  #       sum over every output but the right one
  #
  # Softmax: L_i = - log (exp(s_yi) / sum_j exp(s_j))
  #
  # Chain rule:
  #
  # dL = dL/dw * dw + dL/dx * dx + dL/db * db
  #
  # Back prop is an instance of the method of Lagrange multipliers,
  # which uses efficient block-coordinate steps on the multipliers
...
def batchnorm_backward_alt(dout, cache):
  ...
  dbeta = np.sum(dout, axis=0)
  dgamma = np.sum((x - mu) * 1.0/np.sqrt(var + eps) * dout, axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  # Add padding to each image, (# images, # channels, height, width)
